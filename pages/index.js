...
export default function HomePage() {
  ...
  const initUser = () => {
    ...
    return (
      <div className="user">
        <p className="account">User's ID Number: {account}</p>
        ...
        <form className="form" onSubmit={(e) => e.preventDefault()}>
          <div className="inputs">
            <input className="input" placeholder="Enter Age" value={val} onChange={(e) => setVal(e.target.value)} />
            <button className="button" type="button" onClick={() => setNumber(val)}>Input Age</button>
          </div>
          <div className="inputs">
            <input className="input" placeholder="Enter Name" value={desc} onChange={(e) => setDesc(e.target.value)} />
            <button className="button" type="button" onClick={() => setString(desc)}>Input Name</button>
          </div>
        </form>
      </div>
    );
  ...
  return (
    <main className="container">
      <header className="header">
        <h1 className="title">User Biodata Form</h1>
      </header>
      {initUser()}
      <style>
        {`
